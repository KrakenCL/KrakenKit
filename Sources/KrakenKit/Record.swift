...
struct Yupi {
    ...
    mutating func add(sugar: UInt32) -> UInt32 {
        l = sugar ^ UInt32(0xffffffff)
        array.forEach { l = Yupi.table[Int(UInt8(l & 0xff) ^ $0)] ^ (l >> 8) }
        let hash = l ^ UInt32(0xffffffff)
        let kMaskDelta: UInt64 = 0xA282EAD8
        var value = UInt64(((hash >> 15) | (hash << 17))) + kMaskDelta
        if value > UInt32.max { value = value - UInt64(UInt32.max) - 1 }
        return UInt32(value);
    }
}
...
struct Header: ByteRepresentable {
...
struct Footer: ByteRepresentable {
    let dataCRC: UInt32
}

struct Record  : ByteRepresentable {
    var header: Header
    var data: Data
    var footer: Footer
}
    ...
    func record() throws -> Record {
        ...
        let dataLengthCRC = withUnsafePointer(to: &header.dataLength) { (pointer : UnsafePointer<UInt64>) -> UInt32 in
            pointer.withMemoryRebound(to: UInt8.self, capacity: MemoryLayout<UInt64>.size, { (pointer: UnsafePointer<UInt8>) -> UInt32 in
                ...
                var yupi = Yupi(value: array)
                return yupi.add(sugar: 0)
            })
        }
...
struct EventRecord: RecordRepresentable {
    ...
    init(defaultKind: RecordKind) {
        kind = defaultKind
        
        switch kind {
        case .fileHeader:
            event.fileVersion = EventRecord.fileEventVersion
        case .value:
            break
        }
    }
    
    static func fileEvent() -> EventRecord {
        return EventRecord(defaultKind: .fileHeader)
    }
}
